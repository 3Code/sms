...
function BuatIsinya( $p) {
 ...
 $tampil=mysql_query("select tbl_rekening.*,tbl_pelanggan.namapel  from tbl_rekening,tbl_pelanggan where tbl_rekening.idpel=tbl_pelanggan.idpel");
  ...
  while ($r = mysql_fetch_array($tampil)) {
   $n++;
	...
	$p->Cell(50, $t, $r['namapel'],1, 0, 'L');
    $p->Cell(35, $t, $r['lembar'],1, 0, 'R');
	$p->Cell(40, $t, number_format($r['biaya']),1, 0, 'R');
	
    $p->Ln($t);
    $jumlah=$jumlah + $r['biaya'] ;
   }
   $p->SetFont('Helvetica', 'B', 11);
   $p->Cell(122, $t, 'Jumlah',1, 0, 'C');
   $p->Cell(40, $t, number_format($jumlah),1, 0, 'R');
} 
...
function BuatHeader($page, $p) {
  ...
  $p->Cell(50, $t, 'Nama Pelanggan', 1, 0 , 'C');
  $p->Cell(35, $t, 'Bulan Tunggakan', 1, 0 , 'C');
  $p->Cell(40, $t, 'Rp Tunggakan', 1, 0 , 'C');
    $p->Ln($t);
}
...
{	$pjg = 150;
	...
	$p->SetY(10);
    $p->SetFont("Helvetica", 'B', 10);
    $p->Cell($pjg, 5, 'LAPORAN REKENING TUNGGAKAN ', 11, 1, 'C');
    $p->SetFont("Helvetica", 'B', 10);
